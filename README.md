<div align="center">
  ...
  <h1 align="center">
    ONDEWO CSI Client C-Sharp
  </h1>
</div>

## Overview

`Ondewo.CSI.Client` is a compiled version of the
[ONDEWO CSI API](https://github.com/ondewo/ondewo-csi-api) — the gRPC interface to ONDEWO
Conversational Speech Interface — generated with the
...
over the `ondewo-csi-api` submodule, and it is regenerated in full by `make build`.

## Installation

The package is published to [nuget.org](https://www.nuget.org/packages/Ondewo.CSI.Client) as
**`Ondewo.CSI.Client`**. No custom feed, credential or `nuget.config` entry is needed — the default
`nuget.org` source is enough.

With the .NET CLI, from the directory of the project that should consume it:

```shell
dotnet add package Ondewo.CSI.Client
```

That resolves the latest stable version. To pin one — which is what you want in a service, because
the client version tracks the ONDEWO CSI API in major and minor:

```shell
dotnet add package Ondewo.CSI.Client --version 5.5.0
```
...
```xml
<ItemGroup>
  <PackageReference Include="Ondewo.CSI.Client" Version="5.5.0" />
</ItemGroup>
```
...
```powershell
Install-Package Ondewo.CSI.Client -Version 5.5.0
```
...
- **What else is in the assembly.** `ondewo-csi-api` imports the NLU, S2T and T2S protos directly,
  so the package carries the `Ondewo.Nlu`, `Ondewo.S2T`, `Ondewo.T2S` and
  `Google.Cloud.Dialogflow.V2` stubs alongside `Ondewo.Csi` — it is self-contained and needs no
  sibling ONDEWO package. For the same reason do **not** reference `Ondewo.NLU.Client`,
  `Ondewo.S2T.Client` or `Ondewo.T2S.Client` next to it: both assemblies would define the same
  types and every use of one becomes CS0433 ("the type exists in both").
...
- **Versioning.** `Ondewo.CSI.Client` **5.5.x** is generated from ONDEWO CSI API **5.5.0**: major
  and minor always match the API, the patch number is this client's own.

From source:

```shell
git clone https://github.com/ondewo/ondewo-csi-client-csharp.git   ## Clone the repository
cd ondewo-csi-client-csharp                                        ## Change into the repo directory
...
// `package ondewo.csi;` becomes `Ondewo.<Pascal>` — check the `namespace` line at the top of
...
├── ondewo-csi-api                               <----- submodule: the .proto sources
├── ondewo-proto-compiler                    <----- submodule: the code generator
├── Ondewo.CSI.Client.csproj     <----- generated project file (tracked)
...
| `generate_ondewo_protos`              | runs that image over `ondewo-csi-api/ondewo` and writes the library back into the repo  |
...
- The generated `Ondewo.CSI.Client.csproj` is **tracked**. On the next run the image finds it in the
...
The same glob is why `Ondewo.CSI.Client.csproj` carries
...
Bump `ONDEWO_CSI_VERSION` in the `Makefile`, add a `RELEASE.md` entry in the existing format under a
`## Release ONDEWO CSI Csharp Client <version>` heading — `build_gh_release` slices the release notes out by
...
Pushing a version tag (`5.5.0`, `5.5.0-rc.1` — the shape `make create_release_tag` writes) also triggers
