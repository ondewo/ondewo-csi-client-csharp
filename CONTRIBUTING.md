...
Almost everything in this repository is generated. `api/` and `Ondewo.CSI.Client.csproj` are the
output of the `ondewo-csharp-proto-compiler` docker image running over the `ondewo-csi-api` submodule, and they are
overwritten in full by `make build`.

- A change to the **API surface** (a message, a field, an RPC) belongs in
  [ondewo-csi-api](https://github.com/ondewo/ondewo-csi-api), not here. Once it is released there, bump
  `ONDEWO_CSI_API_GIT_BRANCH` in the `Makefile` and run `make build`.
