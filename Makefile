...
# =====================================================================================
# ondewo-csi-client-csharp - Makefile
#
# Single entry point for regenerating the gRPC client stubs from the ondewo-csi-api protos and
...
#
# Versioning: ONDEWO_CSI_VERSION (below) is the single source of truth. It MUST
# match the ONDEWO CSI API in major and minor version. It is handed to MSBuild as
# the $(OndewoPackageVersion) property, so no file has to be rewritten when it changes - only
# this Makefile and RELEASE.md.
#
# Overriding variables: pass on the command line, e.g. `make build ONDEWO_CSI_VERSION=1.2.3`,
...
# ---------------- BEFORE RELEASE ----------------
# 1 - Update Version Number (ONDEWO_CSI_VERSION)
# 2 - Update RELEASE.md
# 3 - make build
...
CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO CSI Csharp Client ${ONDEWO_CSI_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-csi-client-csharp"
...
OndewoPackageId=Ondewo.CSI.Client
OndewoPackageVersion=${ONDEWO_CSI_VERSION}
...
NUPKG_DIR=nupkg
NUPKG=${NUPKG_DIR}/${OndewoPackageId}.${ONDEWO_CSI_VERSION}.nupkg
SNUPKG=${NUPKG_DIR}/${OndewoPackageId}.${ONDEWO_CSI_VERSION}.snupkg
...
install_precommit_hooks: ## Installs pre-commit hooks and sets them up for the ondewo-csi-client-csharp repo
...
TEST: ## Diagnostics - print the resolved build configuration and the current release notes
	...
	@echo "API submodule pin:    ${ONDEWO_CSI_API_GIT_BRANCH}"
...
build: clean update_submodules checkout_defined_submodule_versions build_compiler generate_ondewo_protos check_build ## Build the client library: submodules -> compiler image -> stubs -> NuGet package
	@echo "$(GREEN)[SUCCESS]$(NC) ${OndewoPackageId} ${ONDEWO_CSI_VERSION} built"
...
generate_ondewo_protos: ## Generate the csharp gRPC client stubs and the NuGet package from the API protos
	...
	docker run --rm \
		-e OndewoPackageVersion=${ONDEWO_CSI_VERSION} \
...
TEST_PROJECT=tests/Ondewo.CSI.Client.Tests/Ondewo.CSI.Client.Tests.csproj
...
release: ## Automate the entire release process
	@echo "$(BLUE)[INFO]$(NC) Start release ${ONDEWO_CSI_VERSION}"
...
# exists and cannot be moved.
	...
	if [ -z "$$notes" ]; then \
		echo "$(RED)[ERROR]$(NC) RELEASE.md has no 'Release ONDEWO CSI Csharp Client ${ONDEWO_CSI_VERSION}' entry"; \
...
publish: publish_dry_run push_to_nuget ## Verify the packed package, then publish it to nuget.org
	@echo "$(GREEN)[SUCCESS]$(NC) ${OndewoPackageId} ${ONDEWO_CSI_VERSION} published to ${NUGET_SOURCE}"
...
verify_nupkg_metadata: ## Assert the packed .nupkg/.snupkg carry the metadata nuget.org expects
	...
	require "Version"                "<version>${ONDEWO_CSI_VERSION}</version>"       "$$nuspec" ; \
...
verify_nupkg_installs: ## Restore the packed package into a throwaway consumer project
	...
	@printf '%s\n' \
		...
		'  </PropertyGroup>' \
		'  <ItemGroup>' \
		'    <PackageReference Include="${OndewoPackageId}" Version="${ONDEWO_CSI_VERSION}" />' \
		'  </ItemGroup>' \
		'</Project>' > ${DRY_RUN_DIR}/consumer/consumer.csproj
	@echo "$(BLUE)[INFO]$(NC) Restoring ${OndewoPackageId} ${ONDEWO_CSI_VERSION} from ${CURDIR}/${NUPKG_DIR} into a throwaway consumer ..."
	...
	@test -d "${DRY_RUN_DIR}/packages/$$(echo ${OndewoPackageId} | tr 'A-Z' 'a-z')/${ONDEWO_CSI_VERSION}" || { \
		echo "$(RED)[ERROR]$(NC) the restore succeeded but did not install ${OndewoPackageId} ${ONDEWO_CSI_VERSION}"; \
		exit 1; \
	}
	@echo "$(GREEN)[SUCCESS]$(NC) a consumer can install ${OndewoPackageId} ${ONDEWO_CSI_VERSION} and its dependency graph"
...
push_to_nuget: ## Publish the packed NuGet package to nuget.org
	...
	@echo "$(BLUE)[INFO]$(NC) Pushing ${OndewoPackageId} ${ONDEWO_CSI_VERSION} (+ symbols) to ${NUGET_SOURCE} ..."
