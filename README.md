...
```xml
<PackageReference Include="Ondewo.CSI.Client" Version="1.0.0" />
```
...
to the tracked `Ondewo.CSI.Client.csproj` before you create `tests/`, otherwise the test sources are
compiled into the library itself and the build fails on the missing test-framework references.

## Releasing

Bump `ONDEWO_CSI_VERSION` in the `Makefile`, add a `RELEASE.md` entry in the existing format, then:
